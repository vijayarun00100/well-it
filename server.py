...
def test():
    ...
    if filtered_data.empty:
        ...
        if ( avg_flu >= 0 ):
            ...
            return render_template("result.html",fluctuation_may = float(y_may_flu[0]), water_may = float(y_may_bgl[0]), fluctuation_aug = float(y_aug_flu[0]), water_aug = float(y_aug_bgl[0]), fluctuation_nov = float(y_nov_flu[0]), water_nov = float(y_nov_bgl[0]), fluctuation_jan = y_jan_flu[0], water_jan = float(y_jan_bgl[0]), depth_fin = float(depth[0]),name="vijay")
        else:
